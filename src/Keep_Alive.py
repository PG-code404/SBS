...
AUTHORIZED_EMAILS = [e.strip() for e in os.getenv("AUTHORIZED_EMAILS", "").split(",") if e.strip()]
...
if GOOGLE_CLIENT_ID and GOOGLE_CLIENT_SECRET:
    ...
    google = None
    logger.warning("⚠️ Google OAuth not configured. Login will be disabled.")
...
def dashboard():
    ...
    now_dt= datetime.now()
    uptime = str(timedelta(seconds=int((now_dt - main.PROCESS_START_TIME).total_seconds())))
...
def pending_schedules():
    ...
    rows = fetch_pending_schedules()  # make sure this sets row_factory = sqlite3.Row
   
    schedules = [
        {
            "id": r["id"],
            "start_time": to_local(r["start_time"]),
            "end_time": to_local(r["end_time"]),
            "target_soc": r["target_soc"] if r["target_soc"] is not None else '-',
            "price_p_per_kwh": r["price_p_per_kwh"] if r["price_p_per_kwh"] is not None else '-',
            "manual_override": bool(r["manual_override"]) if r["manual_override"] is not None else False,
            "source": r["source"] if r["source"] else "scheduler",
            "mode": r["mode"] if r["mode"] else "autonomous",
        }
        for r in rows
    ]
...
def delete_schedule(schedule_id: int):
    ...
    try:
        ...
        return jsonify({"status": "ok", "message": f"Schedule {schedule_id} is deleted"})
    except Exception as e:
